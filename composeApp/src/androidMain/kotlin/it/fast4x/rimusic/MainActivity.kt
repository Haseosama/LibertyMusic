...
//,PersistMapOwner
{
    ...
    // Objects.requireNonNull(sensorManager) returns a Java platform type, so kotlinc's
    // "unnecessary safe call" warning below is a false positive: removing the `?.`
    // is a real compile error ("nullable receiver of type 'SensorManager?'").
    ...
    fun startApp() {
        ...
        setContent {
            ...
            LaunchedEffect(intentUriData) {
                ...
                Toaster.n(
                    "${AppInfo.NAME} ${this@MainActivity.resources.getString( R.string.opening_url )}",
    ...
    // window.statusBarColor/navigationBarColor are deprecated in favor of edge-to-edge
    // APIs, but both calls below are already gated to pre-M/pre-O devices that predate
    // those APIs — this is the intentional legacy fallback, not a missed migration.
