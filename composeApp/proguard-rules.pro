...
# UpdateFeatureHolder instantiates the flavor's implementation by reflection,
# using a class name declared in AndroidManifest.xml <meta-data> — R8 can't see
# that reference statically, so it needs to be told explicitly to keep it.
-keep class * implements app.kreate.android.updater.UpdateFeature { <init>(); }
