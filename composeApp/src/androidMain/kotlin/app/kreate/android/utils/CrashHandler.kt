...
class CrashHandler(
    ...
    companion object {
        ...
        val fileNameRegex = Regex("^${AppInfo.NAME}_crashlog_(\\d{4}-\\d{2}-\\d{2}_[0-2]\\d-[0-5]\\d-[0-5]\\d).log$")
    ...
    override fun uncaughtException( t: Thread, e: Throwable ) {
        ...
        if( AppInfo.isDebug ) e.printStackTrace()
        ...
        val logFile = crashlogsDir.resolve(
            "${AppInfo.NAME}_crashlog_$datetime.log"
        )
        if( !logFile.exists() )
        ...
        context.contentResolver.openOutputStream( logFile.toUri() )?.use { outStream ->
            ...
            writer.println( "Version: ${AppInfo.versionName}" )
            writer.println( "Architect: ${Build.SUPPORTED_ABIS.firstOrNull()}" )
