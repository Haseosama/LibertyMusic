    ...
    SharedPreferences.OnSharedPreferenceChangeListener
{
    ...
    private fun normalizeLoudness() {
        ...
        // loudnessEnhancer stays enabled at all times (see onAudioSessionIdChanged) so an
        // insert audio effect is always attached to the session. On some devices/OEM audio
        // HALs, a session with zero insert effects gets routed through an offloaded/direct
        // output path whose volume handling is broken (system + in-app volume both report
        // 0/muted, yet audio is still audible at full level). When the user's own loudness
        // normalization preference is off, just keep the gain neutral (0 mB = no audible
        // change) instead of disabling the effect entirely.
    ...
    override fun onAudioSessionIdChanged( audioSessionId: Int ) {
        ...
        try {
            ...
            // Always enabled (see normalizeLoudness) — gain is neutral (0 mB) when the
            // user's normalization preference is off, so this stays inaudible.
        ...
        try {
            ...
            // Only enabled when the user actually picked a preset (PRESET_NONE = 0):
            // on some devices this effect is realized as a hardware-tunneled/offloaded
            // implementation whose gain doesn't correctly follow the stream/app volume,
            // so keeping it enabled unconditionally could make playback ignore volume
            // changes even when no reverb is actually selected.
            reverb.enabled = Preferences.AUDIO_REVERB_PRESET.value != 0
    ...
    override fun onSharedPreferenceChanged( pref: SharedPreferences, key: String? ) {
        when( key ) {
            // loudnessEnhancer.enabled stays true regardless of this preference
            // (see normalizeLoudness) — toggling it only changes the target gain.
            Preferences.Key.AUDIO_VOLUME_NORMALIZATION -> normalizeLoudness()
            ...
            Preferences.Key.AUDIO_REVERB_PRESET -> {
                if( ::reverb.isInitialized )
                    reverb.enabled = pref.getInt(key, 0) != 0
                updateReverb()
            }
        }
    }
