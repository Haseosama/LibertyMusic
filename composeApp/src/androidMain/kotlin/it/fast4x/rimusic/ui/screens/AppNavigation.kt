...
import it.fast4x.rimusic.extensions.games.gravity.GravityEasterEgg
...
fun AppNavigation(
    ...
    UpdateFeatureHolder.feature?.UpdateHandler()
    ...
    NavHost(
        ...
        composable(route = NavRoutes.gameGravity.name) {
            modalBottomSheetPage {
                GravityEasterEgg()
            }
        }
    ...
    if( Preferences.SEEN_CHANGELOGS_VERSION.value != AppInfo.versionName ) {
        val changelogs = remember {
            object: ChangelogsDialog(context) {
                ...
                override fun hideDialog() {
                    super.hideDialog()
                    Preferences.SEEN_CHANGELOGS_VERSION.value = AppInfo.versionName
                }
            }
        }
        changelogs.Render()
